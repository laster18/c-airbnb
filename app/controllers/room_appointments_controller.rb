class RoomAppointmentsController < ApplicationController
  ...
  def appointment_params
    params.require(:room_appointment).permit(:message).merge(check_in_date: session[:check_in_date], check_out_date: session[:check_out_date], number_of_adult: session[:number_of_adult], number_of_child: session[:number_of_child], number_of_infant: session[:number_of_infant], room_id: params[:room_id], user_id: current_user.id, total_fee: session[:total_fee])
  end
end
